...
public class TeamsService
{
   ...
   public async Task<List<Team>> GetTeamsForMonitors(User user){
    var result = await _team.Find(x => x.Monitors.Any( y => y.MailAddress == user.MailAddress)).ToListAsync();
    return result;
   }
    ...
    {
        var result = await _team.Find(Team => Team.Leaders.Any(leader => leader.MailAddress == email)).ToListAsync();
        return result;
    }


    public async Task<List<Team>> GetTeamsForDepartmentHead(string email){
        var result = await _team.Find(Team => Team.Monitors.Any(monitor => monitor.MailAddress == email)).ToListAsync();
        return result;
    }

    public async Task<List<User>> GetSupportFromHead(User user)
    {
        var result = await _team.Find(Team => Team.Leaders.Any(x => x.MailAddress == user.MailAddress)).ToListAsync();
    ...
    public async Task<List<User>> GetSupportForDepartmentHead(User user){
        var result = await _team.Find(Team => Team.Monitors.Any(monitor => monitor.MailAddress == user.MailAddress)).ToListAsync();
        List<User> support = new List<User>();
