...
public class UsersService
{
   ...
   public async Task<List<User>> GetSupportUsers(List<Team> teams){
    ...
    var result = await _user.Find( user => teams.Any(team => team.Subordinates.Any( subordinate => subordinate.User.MailAddress == user.MailAddress)) ).ToListAsync();
    ...
    {
        var result = await _user.Find(user => user.EmpName == name).FirstOrDefaultAsync();
        return result;
    }
